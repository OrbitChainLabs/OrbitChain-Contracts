...
fn test_invoke_args_json_to_stellar_argv_integration() {
    ...
    let network = orbitchain_tools::environment_config::EnvironmentConfig::default_for_testnet()
        .testnet;
    ...
    assert!(argv[..sep].windows(2).any(|w| w[0] == "--send" && w[1] == "no"));
    assert_eq!(argv[sep + 1], "donate");

    let method_argv = &argv[sep + 2..];
    let has_pair = |flag: &str, value: &str| {
        method_argv.windows(2).any(|w| w[0] == flag && w[1] == value)
    };
    assert!(has_pair("--donor", donor));
    assert!(has_pair("--campaign_id", "7"));
    assert!(has_pair("--amount", "170141183460469231731687303715884105727"));
